...
public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Tensor& fast = context->input(5);
    ...
    const TensorShape& s_shape = s.shape();
    const TensorShape& t_shape = t.shape();
    const TensorShape& fast_shape = fast.shape();
    ...
    Tensor* grad_fast = NULL;
    OP_REQUIRES_OK(context, context->allocate_output(2, grad_fast_shape, &grad_fast));
